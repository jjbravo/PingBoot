...
public class ConexionBT {
    ...
    // Le enviamos al Handler el nuevo estado actual para que se actualize en la Actividad
        mHandler.obtainMessage(CarLeds.Mensaje_Estado_Cambiado, estado, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(CarLeds.Mensaje_Nombre_Dispositivo);
        Bundle bundle = new Bundle();
        bundle.putString(CarLeds.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(CarLeds.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(CarLeds.TOAST, "Error de conexi�n");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(CarLeds.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(CarLeds.TOAST, "Se perdio conexion");
        msg.setData(bundle);                       
        mHandler.sendMessage(msg);             
        msg = mHandler.obtainMessage(CarLeds.MESSAGE_Desconectado);                   
        mHandler.sendMessage(msg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(CarLeds.Mensaje_Escrito, -1, -1, buffer).sendToTarget();
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>        
 ...
 // String readMessageX = new String(readBufX, 0, bytes);//Se envia el readNessagexxx en lugar del buffer pues ya se PARSEO               
      mHandler.obtainMessage(CarLeds.Mensaje_Leido, bytes, -1, buffer).sendToTarget(); //readMessageX por buffer              
